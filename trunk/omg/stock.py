...
class stock_location(osv.osv):
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        gid = self.pool.get('res.groups').search(cr, uid, [('name','=','OMG / Location Approval')])
        if gid: #In Location Approval
            cr.execute("select gid from res_groups_users_rel where uid=%s and gid=%s " %(uid, gid[0]))
