...
<form name="myform" method="get" action="./store">
	<select name="chain"> %s </select>
	<input type="submit" value="Find" name="find"/>
	<input type="button" name="CheckAll" value="Check All" onClick="checkAll(document.myform.store)">
	<input type="button" name="UnCheckAll" value="Uncheck All" onClick="uncheckAll(document.myform.store)">
	<br/>
	<table>
		%s
	</table>
	...
	if store:
		for i in range(len(store)):
			...
			for location in results:
				...
				loc_args = [('location_id', '=', int(str(location['id']))),('location_set_id','=',int(str(rid)))] 
				loc_ids = sock.execute(dbname, uid, pwd, 'stock.location.set.line', 'search', loc_args)		
				loc_field = ['id','name']
				loc_result = sock.execute(dbname, uid, pwd, 'stock.location.set.line', 'read', loc_ids, loc_field)		
				if not loc_result:				
