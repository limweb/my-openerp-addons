...
class wizard_ineco_dispatch_kitting_summary(osv.osv_memory):
    ...
    def create_report(self, cr, uid, ids, context=None):
        ...
        host_ids = self.pool.get('ineco.report.config').search(cr, uid, [('type','=','report.ineco.dispatch.kitting.summary')])
        hostname = 'localhost'
        if host_ids:
            ...
            raise osv.except_osv(_('Error !'),_('You have not config report report.ineco.dispatch.kitting.summary in ineco.report.config!'))
    ...
    _name = "wizard.ineco.dispatch.kitting.summary"
    _description = "Print Dispatch & Kitting Report"
    ...
    _defaults = { 
        ...
        'name': lambda *a: 'dispatch_kitting.pdf',
        'format': 'pdf'
    }    

wizard_ineco_dispatch_kitting_summary()
