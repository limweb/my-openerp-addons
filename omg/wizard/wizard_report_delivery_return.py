...
class wizard_report_delivery_return_by_period(osv.osv_memory):
    ...
    def create_report(self, cr, uid, ids, context=None):
        ...
        datas['model'] = 'wizard.ineco.report.delivery.return'
        ...
        chain_id = datas['form']['chain_id']
        report_name = datas['form']['name']

        host_ids = self.pool.get('ineco.report.config').search(cr, uid, [('type','=','report.delivery.return')])
        hostname = 'localhost'
        if host_ids:
            ...
            if len(period_id) == 1:
                period_str = ' sp.period_id in (' + str(period_id[0]) +')'
            else:
                period_str = ' sp.period_id in '+ str(tuple(period_id))
                
            a = j.runReport(report_path, this.format,{'period_id': str(period_str),'chain_id': str(chain_id),'product_id':str(customer_product_id)})
            ...
            raise osv.except_osv(_('Error !'),_('You have not config report report.delivery.return in ineco.report.config!'))
    ...
    _name = "wizard.ineco.report.delivery.return"
    _description = "Delivery & Return Report"

    _columns = {
        'name': fields.char('Filename', 16, readonly=True),                
        'period_id': fields.many2many('omg.sale.period', 'report_category_rel', 'report_id', 'period_id', 'Periods'),
        'customer_product_id': fields.many2one('product.product','Brand', required=True),
    ...
    _defaults = { 
        ...
        'name': lambda *a: 'delivery_return_by_period.pdf',
        'format': 'pdf'
    }    


wizard_report_delivery_return_by_period()
